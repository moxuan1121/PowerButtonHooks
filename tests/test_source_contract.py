...
class SourceContractTest(unittest.TestCase):
    ...
    def test_low_power_mode_uses_lock_events_without_polling(self):
        ...
        self.assertIn("if (!NSProcessInfo.processInfo.lowPowerModeEnabled)", SOURCE)
        self.assertIn("PBHLowPowerEnabledByPlugin = PBHSetLowPowerMode(YES);", SOURCE)
        self.assertIn("if (PBHLowPowerEnabledByPlugin && NSProcessInfo.processInfo.lowPowerModeEnabled)", SOURCE)
        ...
        self.assertIn('objc_getClass("SBCoverSheetPrimarySlidingViewController")', SOURCE)
        self.assertIn("PBHCoverSheetDidDisappear", SOURCE)
        self.assertIn("@selector(viewDidDisappear:)", SOURCE)
        self.assertIn("PBHUsesCoverSheetDismissalHook", SOURCE)
        self.assertIn("if (PBHIsActualUIUnlock()) PBHHandleUILockState(NO);", SOURCE)
        self.assertNotIn("PBHFinishUIUnlock", SOURCE)
        self.assertIn("if (PBHLowPowerSessionActive)", SOURCE)
        self.assertIn("PBHLowPowerSessionActive = NO;", SOURCE)
